...
// ============================================================
// RESPONSIVE SESSION LAYOUT — stacks on mobile, splits into
// info | session columns on wide / landscape viewports
// ============================================================
function SessionLayout({ phase, info, children }) {
  return (
    <div className="session-layout">
      <div className="session-layout-info" data-phase={phase}>{info}</div>
      <div className="session-layout-main">{children}</div>
    </div>
  );
}
...
function KapalabhatiScreen({ exercise, soundOn, sounds, onComplete }) {
  ...
  return (
    <div style={{ padding: '4px 4px 30px' }}>
      <SessionLayout phase={phase} info={<InfoBlock exercise={exercise} />}>
        {phase !== 'ready' && (
          <RoundTracker
            current={round}
    ...
    <div style={{
      ...
      <style>{`
        ...
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        /* Page shells: phone-width column by default, wider on landscape/desktop
           so there's room for the two-column session layout to actually spread out. */
        .home-screen-shell {
          max-width: 520px;
          margin: 0 auto;
          padding: 0 16px 40px;
        }
        .exercise-screen-shell {
          max-width: 520px;
          margin: 0 auto;
          padding: 0 16px;
        }
        ...
        /* Mobile default: info and session stack vertically, in document order. */
        .session-layout {
          display: flex;
        ...
        /* On mobile, hide the info pane once a session has moved past 'ready' —
           keeps the cramped phone view focused on the active timer, same as before. */
        .session-layout-info[data-phase]:not([data-phase="ready"]) {
          display: none;
        }

        /* Landscape / desktop: side-by-side columns, info stays visible
           throughout the session instead of disappearing once started. */
        @media (min-width: 860px) {
          .session-layout {
            display: grid;
